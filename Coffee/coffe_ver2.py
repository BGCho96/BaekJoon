...
for n in range(1,K+1):
    ...
    for k in range(len(caffeine_vol)):
        ...
        #첫잔일때
        if k==0:
            if caffeine_vol[k]>n or caffeine_vol[k]==0:
                status_K=[n,0]
            else: status_K=[n-caffeine_vol[k],1]
        #아닐때
        elif caffeine_vol[k]==0:
            status_K=status_N[-1]
        else:
            if caffeine_vol[k]>n:
                status_K=status_N[-1]
                ...
                if newCoffee[0]<status_N[-1][0]:# 카페인이 동일하다면 잔수 이득만
                    status_K=newCoffee
                if (newCoffee[0]==status_N[-1][0]) and (newCoffee[1]<status_N[-1][1]):status_K=newCoffee
